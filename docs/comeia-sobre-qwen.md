# 🐝 A COMEIA sobre o Qwen3.5-4B — o método, medido

> ⚠️ **ESTE É O HISTÓRICO, NÃO O PROJETO ATUAL.** Tudo aqui roda sobre o **Qwen3.5-4B (Alibaba)** —
> o modelo é deles, o método é nosso. Em 2026-07-26 o projeto voltou ao objetivo original —
> **construir uma LLM do zero** — e passou a se chamar **Bee**. Ver o [README](../README.md).
>
> **Por que este documento continua no repositório:** os resultados abaixo são reais, medidos e, em
> vários casos, negativos — e é justamente o método que produziu esses números (gate → filtro "o base
> erra" → holdout duplo → baseline externo) que será **reaplicado sobre o Bee**. Apagar isto seria
> perder a parte mais cara do aprendizado. Os erros estão registrados de propósito.
>
> **O que NÃO transfere:** os três adapters treinados aqui estão atados ao Qwen e morrem com a troca
> de backbone. O que transfere é o pipeline, o harness e as regras.
...
Reprodução verificada: `comeia/colab/reproduce_extracao.py` reconstruiu a abelha em **53,8 min** num
...
- **`comeia/colab/reproduce_extracao.py`** — a abelha inteira em ~35 min, um comando, retomável. É a defesa
...
**Conserto estrutural, não remendo:** `comeia/data/tool_catalog.py` vira a fonte única; treino, avaliação e
...
```bash
python comeia/colab/reproduce_coder.py --out /content/qwen35-4b-coder   # ou um caminho no Drive
```
...
```bash
CODER_ADAPTER=models/coder AGENTICA_ADAPTER=models/agentica python comeia/orchestrator/run.py --sample
```
...
Medimos o **base ANTES de treinar** (`comeia/eval/eval_coder.py`, pass@1 por execução, 60 tarefas):
...
**Precisão medida ANTES de ligar** (`comeia/orchestrator/test_verifier.py`, 150 exemplos rotulados, sem GPU):
...
(`comeia/eval/eval_agentic.py`, 60 exemplos: 35 que exigem ferramenta + 25 que não exigem):
...
clássica de modelo agêntico. Validação dura contra o catálogo (`comeia/data/agentic_tools.json`):
...
## Estrutura
```
├── comeia/orchestrator/   # 🐝 A COMEIA — registry, engine (hot-swap), router, CLI
...
├── comeia/data/           # pipeline de dados (destilação, self-instruct, preferências, agêntico)
├── comeia/train/          # SFT e DPO via QLoRA
├── comeia/eval/           # harness PT-BR (lm-eval), comparação com significância, gate QuantiBias
├── comeia/colab/          # receita da L4 (Colab Pro+)
...
```bash
python comeia/orchestrator/run.py --sample --max-new 256
python comeia/orchestrator/run.py --chat-adapter models/qwen3.5-4b-ptbr-sft   # adapter local
```

**Fabricar uma abelha nova** (exemplo: a agêntica):
```bash
python comeia/data/07b_expand_agentic.py --target 1500          # sementes: 91 -> 1690
python comeia/data/07_distill_agentic.py --seeds comeia/data/seeds_agentic_expanded.txt --workers 16
python comeia/data/05_build_splits.py --in comeia/data/raw/agentic_<prof>.jsonl \
    --out comeia/data/processed/sft_agentic.jsonl --system-file comeia/data/agentic_system.txt
python comeia/train/sft_qlora.py --data comeia/data/processed/sft_agentic.jsonl    # -> adapter
# registrar o adapter_path no comeia/orchestrator/bees.json
```
...
`deepseek-v4-flash`. Rodar `python comeia/data/00_check_teachers.py` regenera a tabela de preços.
